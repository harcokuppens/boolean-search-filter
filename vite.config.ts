...
// Simple Vite plugin to list all HTML files in the examples directory instead of a specific index.html
// when accessing the root URL. This allows easy navigation to all example files during development.
function directoryListingPlugin(rootDir = '.'): Plugin {
    return {
        name: 'directory-listing',
        configureServer(server) {
            server.middlewares.use((req, res, next) => {
                if (req.url === '/' || req.url === '') {
                    const dirPath = path.join(server.config.root, rootDir);
            ...
            <body style="font-family:sans-serif;max-width:40em;margin:auto;padding:2em">
              <h1>Examples</h1>
              <ul>${links}</ul>
            </body></html>
          `;
...
export default defineConfig(({ command, mode }) => {
    ...
    return {
        // 👇 Treat the `examples` folder as the root for dev server (not when building)
        root: isDev ? 'examples' : '.', // use examples/ only in dev mode

        // Simple dev server directory listing plugin -> we can see all example html files in the browser
